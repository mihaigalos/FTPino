...
typedef enum TEFileSystem{
    TEFileSource_Unknown,    TEFileSource_Eeprom,    TEFileSource_SdCard
};

typedef enum TEFileType{
    TEFileType_Unknown, TEFileType_File, TEFileType_SymbolicLink, TEFileType_Folder
};

class IFileHandler; class FilePropeties;
...
public:    
	unsigned char buffer [1024]; size_t byteCount;  
	PrintHelper(){memset(buffer,0, sizeof(buffer));}
	unsigned char* c_str(String s){
		s.getBytes(buffer, sizeof(buffer));
		return &buffer[0];
	}
	size_t write(uint8_t character){ if(byteCount >= sizeof(buffer))  byteCount = 0;                      buffer[byteCount++] = character;    return 1;}
...
public:
    ...
    virtual int writeFile(String file, char* contents, uint16_t length, bool isAppend=false)     = 0;
...
class FilePropeties{
    ...
    TEFileType  type;
    String      contents;
    friend class Eeprom_FileHandler; friend class SDCard_FileHandler;  friend class IFileHandler;    
};
...
public: 
    ...
    int writeFile(String file, char* contents, uint16_t length, bool isAppend=false);
